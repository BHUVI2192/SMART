...
import { ArrowLeft, Video, Loader2, Sparkles, XCircle, CheckCircle } from 'lucide-react';
...
type ScanStage = 'PERMISSION' | 'CAMERA' | 'PROCESSING' | 'RESULT';
...
const ScanPage: React.FC<ScanPageProps> = ({ user, authUser }) => {
  ...
  const startLiveCamera = async () => {
    ...
    try {
      ...
      setCameraError('Camera access denied. Please check permission settings.');
      setCameraActive(false);
    }
  };
      ...
      if (apiReady) {
        ...
        if (apiResult.success) {
          ...
          switch (apiResult.code) {
            ...
            default: setResult('FAIL_ERROR'); setResultMessage(apiResult.error || 'Verification failed.');
          }
        }
      } else {
        ...
        if (sessionData.token !== qrData) { setResult('FAIL_INVALID_QR'); setResultMessage('QR code has expired.'); setStage('RESULT'); return; }
        const newLog = {
  ...
  const ResultCard: React.FC<{
    ...
    <div className="text-center w-full max-w-sm mx-auto animate-scale-in">
      <div className={`candy - card p - 6 sm: p - 8`}>
        ...
        {result === 'SUCCESS' && (
          <p className="text-[10px] text-slate-400 mb-4">Verified by Secure Token</p>
        )}
    ...
    <div className="max-w-md mx-auto min-h-[calc(100vh-180px)] flex flex-col animate-fade-in">
      ...
      <div className="flex-1 rounded-3xl overflow-hidden relative shadow-2xl shadow-black/20 flex flex-col gradient-dark">
        ...
        <div className="p-5 pb-0 relative z-10 text-white text-center">
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">Point camera at the screen</p>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-white">
          ...
          {stage === 'PROCESSING' && (
            <div className="text-center animate-scale-in">
              ...
              <p className="text-base font-semibold">Verifying Token...</p>
              <p className="text-[11px] text-slate-500 mt-1">Checking with server...</p>
            </div>
          )}

          {/* Removed GPS_CHECK stage */}
          ...
          {stage === 'RESULT' && result === 'FAIL_ERROR' && (
            <ResultCard icon={<XCircle className="w-8 h-8 text-red-600" />} title="Verification Failed" accent="bg-red-100"
              message={resultMessage}
              buttonText="Try Again" buttonAction={() => { setStage('CAMERA'); setResult(null); startLiveCamera(); }}
              buttonStyle="bg-slate-800 text-white hover:bg-slate-700" />
          )}
